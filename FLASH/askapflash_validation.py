...
def get_FIRST(ra, dec):
    ...
    *** For a quick test run, do not activate this cross-matching function
    """
...
def qc_Bad_Chans(infile, mad_rms, med_rms):
    ...
    value = 10*med_rms  # FLASH - deviation from med_rms. current criteria = 10*med_rms, *** need to be discussed 
...
def qc_BeamLogs():
    """
     *** BeamLogs - taken from DINGO, need to be updated for FLASH
...
#   FLASH - test with a single beam (Beam 00)
#   plt.scatter([beamXPOS[21]], [beamYPOS[21]], s=1500, c='none', edgecolors='magenta', lw=3) 
...
def qc_NoiseRank(spread):
    ...
    sigma = 0.5 # for a Gaussian, *** FLASH - need to be discussed
...
def BeamStat_plot(item, n):
    ...
    if item == 'MADMFD':
        vmin = 0.0    
        vmax = 5.0     # FLASH - need to be discussed, current: arbitrary cut
...
#hipass_cat = get_HIPASS(ra, dec) # not activated for FLASH now
...
# NOTE: careful - Flagging_Summaries directory
...
##### to run a quick test - remove this cross-matching 
html.write("""</td> 
              </tr>
              </table>
              <h2 align="middle">FIRST sources within 6 x 6 sq degree (2014Dec17; Helfand+ 2015)</h2>
              <table>
              <tr align="middle">
              <td id="cell">
...
html.write("""</td> 
              ...
              <h2 align="middle">NVSS sources within 6 x 6 sq degree (Condon+ 1998)</h2>
              <table>
              <tr align="middle">
              <td id="cell">
              <form action="{0}" method="get" target="_blank">
