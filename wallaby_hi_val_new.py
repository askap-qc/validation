...
def cal_Beam_TheoRMS(n_ant, tobs, chan_width, sbid):
    """
    Calculating the median of System Equivalent Flux Density (SEFD) (1150-1450 MHz) and the theoretical RMS of each beam. 
...
def cal_Beam_ExpRMS(FLAGSTAT, BEAM_T_RMS):
    """
    ...
    formula = lambda stat, beam_t_rms: 1/math.sqrt(float(1-stat/100)) * beam_t_rms # 1/sqrt(non-flagged fraction) * theoretical rms in mJy
    BEAM_EXP_RMS = [ formula(a, b) for a, b in zip(FLAGSTAT, BEAM_T_RMS) ]
    
    return BEAM_EXP_RMS


def cal_Beam_RatioRMS(BEAM_EXP_RMS):
    ...
    for i in range(36):
        ...
        avgRMS = cal_beam_AvgRMS(infile)
        beam_ratio_RMS = avgRMS/exp_rms[i]
        BEAM_RATIO_RMS.append(beam_ratio_RMS)
        
    return BEAM_RATIO_RMS
...
def Beam_RatioRMSplot(BEAM_RATIO_RMS, n):
    """
    Plotting and visualising RMS ratio (actual/expected) of 36 beams. 
    """

    title = 'RMS Ratio (Actual/Expected)'
    plot_name = 'Ratio_RMS.png'
    saved_fig = fig_dir+'/'+plot_name  
    ...
    for i in range(len(BEAM_RATIO_RMS)):
        bnum = n[i]
        plt.scatter([beamXPOS[i]], [beamYPOS[i]], s=1300, c=[BEAM_RATIO_RMS[bnum]], cmap='summer', edgecolors='black', vmin=0, vmax=1.0)
        plt.text(beamXPOS[i], beamYPOS[i], n[i], va='center', ha='center')
